"""PDFMatcher: match PDF files to articles in journal XML."""
    ...
    def extract_zip(self, zip_path: Path, extract_to: Path) -> Dict[str, Any]:
        ...
        try:
            written = extract_zip_safely(
                ...
                extract_to,
                limits=ZipLimits(),
                name_transform=_decode_zip_filename,
                suffixes=(".xml", ".pdf"),
            )
            for extracted_path, safe_name in written:
                ...
                if suffix == ".xml":
                    if xml_path is None:
                        ...
                        logger.warning(
                            "Найдено несколько XML, используется первый: %s", xml_path.name
                        )
                elif suffix == ".pdf":
