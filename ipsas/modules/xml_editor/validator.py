...
from ipsas.modules.xml_editor.parser import list_articles, parse_article
from ipsas.modules.xml_editor.utils import doi_ok, email_ok, looks_like_email, orcid_ok


def validate_tree_has_articles(tree: etree._ElementTree) -> list[dict[str, str]]:
    issues: list[dict[str, str]] = []
    from ipsas.modules.xml_editor.parser import list_article_elements
        ...
        for i, author in enumerate(authors, start=1):
            ...
            if orcid and not orcid_ok(orcid):
                issues.append(
                    {
                        "severity": "error",
                        "field": f"author_{i}_orcid",
                        "text": f"Автор {i}: некорректный ORCID",
                    }
                )
        if not has_named:
