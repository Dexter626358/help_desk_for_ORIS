...
def _author_orcid(author: etree._Element) -> str:
    """ORCID в journal XML — в authorCodes/orcid (не в individInfo)."""
    codes_orcid = find_one(author, XML_PATHS["author_orcid"])
    if codes_orcid is not None and element_text(codes_orcid):
        return element_text(codes_orcid)
    # Запасной вариант: устаревшее размещение внутри individInfo
    for individ in author.xpath("./individInfo"):
        legacy = find_one(individ, XML_PATHS["orcid"])
        if legacy is not None and element_text(legacy):
            return element_text(legacy)
    return ""
...
def parse_authors(article: etree._Element) -> list[dict[str, Any]]:
    ...
    for idx, author in enumerate(authors_el.xpath("./author")):
        block: dict[str, Any] = {
            ...
            "RUS": _empty_author_lang(),
            "ENG": _empty_author_lang(),
        }
        for individ in author.xpath("./individInfo"):
