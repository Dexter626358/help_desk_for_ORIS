...
from xml_editor.validator import (
    summarize_issues,
...
bp = Blueprint("xml_editor", __name__)
...
def _parse_authors_from_request() -> list[dict[str, Any]]:
    """Собирает авторов из полей authors-<i>-RUS-surname и т.п."""
    indices: set[int] = set()
    for key in request.form:
        if key.startswith("authors-") and key.count("-") >= 3:
            try:
    ...
    for i in sorted(indices):
        block: dict[str, Any] = {"id": str(i), "RUS": {}, "ENG": {}}
        for lang in ("RUS", "ENG"):
            for field in (
                "surname",
                "initials",
...
def index():
    ...
    return render_template("upload.html", max_mb=max_mb)
...
@bp.get("/editor/<session_id>")
@bp.get("/editor/<session_id>/<article_id>")
...
@bp.post("/editor/<session_id>/<article_id>/save")
...
@bp.post("/editor/<session_id>/<article_id>/authors/add")
...
@bp.post("/editor/<session_id>/<article_id>/authors/<author_id>/delete")
...
@bp.post("/editor/<session_id>/<article_id>/authors/<author_id>/move")
...
@bp.post("/editor/<session_id>/<article_id>/restore")
...
@bp.post("/editor/<session_id>/restore-all")
...
@bp.get("/download/<session_id>/original")
...
@bp.get("/download/<session_id>/edited")
