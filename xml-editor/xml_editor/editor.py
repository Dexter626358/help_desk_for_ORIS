...
from xml_editor.parser import get_article_element, list_article_elements
...
def _apply_author_lang(author: etree._Element, lang: str, data: dict[str, Any]) -> None:
    ...
    has_any = any(str(data.get(k) or "").strip() for k in (
        "surname", "initials", "first_name", "middle_name",
        "org_name", "address", "email", "orcid",
    ))
    ...
    _set_individ_field(individ, "orcid", str(data.get("orcid") or ""))
...
def update_article_from_form(
    ...
    if authors_el is not None:
        ...
        # Обновляем существующих по индексу; лишних не удаляем здесь
        # (удаление — отдельный маршрут)
...
def restore_all_from_original(session_edited_path, session_original_path) -> etree._ElementTree:
    from xml_editor.utils import parse_xml_file, save_tree
